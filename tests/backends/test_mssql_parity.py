"""SQL Server parity: the DuckDB behaviours, re-driven over real SQL Server.

Every test here uses the ``mssql_env`` fixture (see :mod:`conftest`), which resets
a throwaway SQL Server to a clean schema and selects the SQL Server backend, then
...
The whole module is marked :mod:`mssql`; run it with ``pytest -m mssql``.
"""
...
@contextmanager
def _mssql_client() -> Iterator[TestClient]:
    """Yield a lifespan-active client bound to the selected SQL Server backend.

    The ``mssql_env`` fixture must already have exported the ``LAVS_DB_BACKEND`` /
    ``LAVS_MSSQL_*`` environment; entering the client runs the application
    lifespan, which builds the SQL Server backend and materialises the schema on
    the clean database.
    """
...
def _create_product(client: TestClient, name: str) -> dict[str, Any]:
    ...
    response = client.post("/products", json={"name": name, "description": "mssql-parity"})
...
class TestProductParity:
    """Products CRUD and the unique-name 409, on SQL Server."""

    def test_create_and_read_back_product(self, mssql_env: Any) -> None:
        # Arrange / Act
        with _mssql_client() as client:
    ...
    def test_duplicate_name_returns_409(self, mssql_env: Any) -> None:
        # Arrange
        with _mssql_client() as client:
...
class TestVersionLifecycleParity:
    """Component + version lifecycle, history, and non-destructive rollback, on SQL Server."""

    def test_history_and_rollback_are_non_destructive(self, mssql_env: Any) -> None:
        # Arrange
        with _mssql_client() as client:
...
class TestTimelineParity:
    """The composite product timeline renders over SQL Server."""

    def test_timeline_lists_component_versions(self, mssql_env: Any) -> None:
        # Arrange
        with _mssql_client() as client:
            product = _create_product(client, "Aurora Platform")
            component = _create_component(client, product["id"], "lavs-api", "service")
...
class TestReleaseParity:
    """Release cut, frozen manifest, immutability, and idempotency, on SQL Server."""
    ...
    def test_cut_freezes_manifest_and_reads_back(self, mssql_env: Any) -> None:
        # Arrange
        with _mssql_client() as client:
    ...
    def test_second_cut_bumps_minor(self, mssql_env: Any) -> None:
        # Arrange
        with _mssql_client() as client:
            seed = self._seed_two_components(client)
    ...
    def test_idempotency_key_collapses_retried_cut(self, mssql_env: Any) -> None:
        # Arrange
        with _mssql_client() as client:
            seed = self._seed_two_components(client)
...
class TestAuthParity:
    """The signup -> verify -> login -> me -> logout flow, on SQL Server."""

    def test_full_auth_flow(self, mssql_env: Any, monkeypatch: pytest.MonkeyPatch) -> None:
        # Arrange -- enforce password auth on top of the already-selected SQL Server backend.
